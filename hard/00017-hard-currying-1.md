...
```ts
type Curry<T> = T extends (...args: infer P) => infer Result // 判断 T 是否是一个函数
? P['length'] extends 0 | 1 // 判断函数的参数个数是否为 0 或 1
  ? T // 如果是，则返回原函数
  : P extends [infer A, ...infer Rest] // 如果不是，则将参数拆分为第一个参数和剩余参数
    ? (arg: A) => Curry<(...args: Rest) => Result> // 将剩余参数递归传入 Curry
    : never
: never
