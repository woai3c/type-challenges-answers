...
type Chainable<Options = {}> = {
  option<K extends string, V>(
    // 如果新增的 key 存在于旧值，则看看它们的新旧值是否一样，如果一样则抛弃这个 key，否则使用这个 key
    key: K extends keyof Options ? (V extends Options[K] ? never : K) : K, 
    value: V
  ): Chainable<{
    // 将新旧值的 key 混合在一起遍历，如果新增的 key 和原有的 key 相同，则使用新值，否则使用旧值
    [Key in keyof Options | K]: Key extends K 
    ? V
