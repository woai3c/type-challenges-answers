...
```ts
type Combination<T extends string[], U = T[number], Item = U> = Item extends string ? Item | `${Item} ${Combination<[], Exclude<U, Item>>}` : never
```

这道题的解题思路和 [3326-BEM style string](https://github.com/woai3c/type-challenges-answers/blob/main/medium/3326-BEM%20style%20string.md) 差不多，需要了解 TS 类型在什么情况下会进行分发。
