...
## 答案
```ts
type ReplaceAll<S extends string, From extends string, To extends string> = S extends `${infer A}${From}${infer B}` // A、B 是任何字符串都行
? From extends '' ? S : `${A}${To}${ReplaceAll<B, From, To>}` // 如果 From 为空，则不替换
: S
```
``${A}${To}${ReplaceAll<B, From, To>}`` 如果有多个子串符合匹配规则 `Form`，则需要进行多次替换。由于是从左到右进行替换，所以替换一次后，只需要对 `B` 重复执行 `ReplaceAll<>` 即可。