...
## 答案
```ts
// 逐个字符比较，先看是否正负号、% 号，然后才是数字
type PercentageParser<A extends string, B extends string[] = ['', '', '']> = A extends `${infer Char}${infer Rest}` 
? (
  Char extends '+' | '-' 
  ? PercentageParser<Rest, [Char, B[1], B[2]]> 
  : Char extends '%' 
    ? [B[0], B[1], '%'] 
    : PercentageParser<Rest, [B[0], `${B[1]}${Char}`, B[2]]>
) 
: B
```