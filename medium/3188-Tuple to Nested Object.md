...
## 答案
从右向左遍历数组，并且要继承类型 `string`。
```ts
type TupleToNestedObject<T extends string[], U> = T extends [...infer R extends string[], infer A extends string] 
? TupleToNestedObject<R, Record<A, U>> 
: U
```