...
declare function PromiseAll<T extends unknown[]>(
  value: readonly [...T]
// 数组、对象都可以使用 in keyof 来遍历 keys.
): Promise<{ [Key in keyof T]: (T[Key] extends Promise<infer A> ? A : T[Key]) }>
```
`value: readonly [...T]` 这里必须转成元组，否则过不了 case1 case2。 只有转成元组，ts 才能精确识别里面的值，否则只能识别为类型，case3 就是一个例子。
```ts
// case3 返回值为 Promise<[number, number, number]>
