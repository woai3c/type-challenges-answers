...
答案一
```ts
type MyReadonly2<T, K extends keyof T = keyof T> = [K] extends never 
? Readonly<T> 
: { readonly [Key in keyof T as (Key extends K ? Key : never)]: T[Key] } & { [Key in keyof T as (Key extends K ? never : Key)]: T[Key] }
```
答案二
```ts
type MyReadonly2<T, K extends keyof T = keyof T> = { readonly [Key in K]: T[Key] } & { [Key in Exclude<keyof T, K>]: T[Key] }
```
`K extends keyof T = keyof T` 为 K 设置默认值，这里的默认值是 T 的 keys 列表。